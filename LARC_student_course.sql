SELECT 
/*course term columns*/
  a.STDNT_ID,a.TERM_CD,a.CLASS_NBR,a.TERM_SHORT_DES,a.CRSE_GRD_OFFCL_CD,
a.GRD_PNTS_PER_UNIT_NBR,a.UNITS_ERND_NBR,a.EXCL_CLASS_CUM_GPA,
a.SBJCT_CD,a.CATLG_NBR,a.CRSE_ID_CD

FROM CNLYR002.CURR_STDNT_TERM_CLASS_INFO a
    LEFT JOIN CNLYR002.CURR_STDNT_TERM_INFO b ON
        a.STDNT_ID=b.STDNT_ID
    LEFT JOIN CNLYR002.CURR_STDNT_INFO c ON
        b.STDNT_ID=c.STDNT_ID AND
        c.FIRST_TERM_ATTND_CD = b.TERM_CD    
WHERE a.INCL_GPA_IND = 1 AND b.PRMRY_CRER_CD LIKE '%U%' AND 
  b.ENTRY_TYP_SHORT_DES = 'Freshman';

/*untested: still need to try running this*/
